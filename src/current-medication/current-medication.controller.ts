...
export class CurrentMedicationController {
    constructor(private readonly currentMedicationService: CurrentMedicationService, private readonly patientService: PatientService,  private readonly doctorService : DoctorService) {}
   ...
   async create(@Body() currentMedicationData: CreateCurrentMedicationDto, @CurrentUser() user: User) {
    ...
    try{
     ...
     const currentMedication = await this.currentMedicationService.create({doctor, doses : currentMedicationData?.doses, startDate : currentMedicationData?.startDate, endDate : currentMedicationData?.endDate})
     ...
     if(!existingPatient){
      ...
      await this.patientService.update(existingPatient.id, {currentMedications : [...existingPatient.currentMedications, currentMedication]})
      return {
       code : '201',
       message : "Current medication data created successfully!",
       data : currentMedication,
